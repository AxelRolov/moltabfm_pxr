...
| [`notebooks/fm_activity_prediction.py`](./notebooks/fm_activity_prediction.py) | marimo port of the foundation-model activity workflow. Uses RDKit 2D, MACCS, Mordred, frozen CheMeleon, fine-tuned CheMeleon embeddings, fine-tuned Chemprop embeddings, ChemBERTa, and MoLFormer with configurable random/scaffold/cluster CV, writes `outputs/my_fm_activity_submission.csv`, and also exports an all-model prediction table. |
...
- a task-tuned Chemprop embedding model trained directly in the activity notebook
